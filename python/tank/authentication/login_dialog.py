...
class LoginDialog(QtGui.QDialog):
    ...
    def __init__(
        ...
        try:
            self._sso_saml2 = SsoSaml2Toolkit("ShotGrid Web Login", qt_modules=qt_modules)
        ...
        self.menu_action_legacy = QtGui.QAction("Authenticate with legacy", menu)
        self.menu_action_legacy.triggered.connect(self._menu_activated_action_legacy)
    ...
    def _toggle_web(self, menu_action=None):
        ...
        if self._query_task.unified_login_flow2_enabled:
            if menu_action:
                # Selecting requested mode (ulf2 or legacy)
                if menu_action == self.menu_action_legacy:
                    # Nothing to do -> either web or legacy depending on use_web
                    pass
                elif menu_action == self.menu_action_ulf2:
        ...
        self.menu_action_legacy.setVisible(self._use_local_browser)

    def _menu_activated_action_ulf2(self):
        self._toggle_web(menu_action=self.menu_action_ulf2)

    def _menu_activated_action_legacy(self):
        self._toggle_web(menu_action=self.menu_action_legacy)
    ...
    def _ulf2_back_pressed(self):
        ...
        if self._ulf2_task and self._ulf2_task.isRunning():
            print("Kill task")
            self._ulf2_task.terminate()
...
import time # TODO - remove callback ...
