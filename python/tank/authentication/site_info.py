...
class SiteInfo(object):
    ...
    def __init__(self, url, http_proxy=None):
        ...
        self.info = {}
        self.info_time = 0 # pre-expire cache


    def _get_site_infos(self):
        """
        ...
        try:
            ...
            http_proxy = _sanitize_http_proxy(self.http_proxy).netloc
            if http_proxy:
                get_logger().debug(
            ...
            if (time.time() - self.info_time) > self.CACHE_TIMEOUT:
                get_logger().info("Infos for site '%s' not in cache or expired", self.url)
                sg = shotgun_api3.Shotgun(
                    self.url, session_token="dummy", connect=False, http_proxy=http_proxy
                )
                ...
                get_logger().info("Infos for site '%s' found in cache", self.url)
        # pylint: disable=broad-except
        except Exception as exc:
            # Silently ignore exceptions
            get_logger().debug("Unable to connect with %s, got exception '%s'", self.url, exc)

        return self.infos


    def _get_user_authentication_method(self):
        """
    ...
    def is_unified_login_flow2_enabled_on_site(self):
        """
    ...
    def is_unified_login_flow_enabled_on_site(self):
        """
    ...
    def is_sso_enabled_on_site(self):
        """
        ...
        """
        return self._get_user_authentication_method() == "saml2"


    # pylint: disable=invalid-name
    def is_autodesk_identity_enabled_on_site(self):
        """
        ...
        """
        return self._get_user_authentication_method() == "oxygen"
